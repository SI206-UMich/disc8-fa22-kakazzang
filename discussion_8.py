...
def getAdmissionsInfo2019(soup):
    school_dict = {}
    schoolInfo = soup.find('table', class_="toccolours")
    tr_list = schoolInfo.find_all('tr')
    for tr_tag in tr_list[1:]:
        td_list = tr_tag.find_all('td')
        school = td_list[0].text.strip()
        year = td_list[1].text.strip()
        school_dict[school] = year
    return school_dict
...
class TestAllMethods(unittest.TestCase):
    ...
    def test_admissions_info(self):
        self.assertEqual(getAdmissionsInfo2019(self.soup), {'Literature, Science, and theArts': '1841',
                                                            'Medicine': '1850',
                                                            'Engineering': '1854', 
                                                            'Law': '1859',
                                                            'Dentistry': '1875', 
